...
def get_data_numeric(max_output):
    ...
    new_timepoints_with_certain_length = [new_timepoints[i] for i in range(len(new_timepoints)) if
                                          len(new_timepoints[i]) > min_length]
    ...
    summed_values_float = np.array(
        [sum(new_values_float[i][k] for i in range(len(new_values_float))) for k in range(min_length)])

    # scale list
    max_summed_values = max(summed_values_float)
    summed_values_scaled = np.array(
        [max_output * summed_values_float[i] / max_summed_values for i in range(len(summed_values_float))])

    # convert to int, timepoints, new number of houses
    return summed_values_scaled.astype(int), new_timepoints_with_certain_length[0], int(
        max_output / max_summed_values * len(values))


def init_chain(max_output):
    """
    initializes a Markov chain and calculates the transition probabilities from the relative frequencies in the data.
    """
